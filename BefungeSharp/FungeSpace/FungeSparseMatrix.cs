...
namespace BefungeSharp.FungeSpace
{
	...
	/// </summary>
	public class FungeSparseMatrixRowEnumerator : IEnumerable<FungeNode>
	{
	...
	/// </summary>
	public class FungeSparseMatrixColumnEnumerator : IEnumerable<FungeNode>
	{
		private FungeNode _traverse;
		private FungeNode _search_origin;
	...
	public class FungeSparseMatrix : IEnumerable<FungeNode>
	{
		...
		/// </summary>
		private List<FungeNode> Nodes { get; set; }
		...
		/// </summary>
		public FungeNode Origin { get; private set; }
		...
		/// <summary>
		/// The matrix bounds of the world, including spaces on the edges
		/// </summary>
		...
		public FungeSparseMatrix()
		{
			...
			Origin = new FungeNode(data, this);
			Origin.ParentMatrix = this;
			Nodes.Add(Origin);

			EnumerationArea = FungeSpaceArea.FS_THEORETICAL;
		...
		public FungeSparseMatrix(List<List<int>> initial_content)
		{
			...
			Nodes = new List<FungeNode>();
			Origin = new FungeNode(new FungeCell(0, 0, ' '), this);
		...
		{
			Nodes = new List<FungeNode>(copy.Nodes.Count);
			Origin = new FungeNode(new FungeCell(0, 0, copy.Origin.Data.value), this);
			Nodes.Add(Origin);

			for (int i = 1; i < copy.Nodes.Count; i++)
			{
				this.InsertCell(copy.Nodes[i].Data);
			}
		}
		...
		public FungeNode GetNode(int row, int column)
		{
			...
			EnumerationArea = new FungeSpaceArea(row, column, FungeSpaceArea.FS_THEORETICAL.bottom, FungeSpaceArea.FS_THEORETICAL.right);
			foreach (var node in this)
			{
				if (node.Y == row && node.X == column)
				{
		...
		public FungeNode GetNode(Vector2 position)
		{
			return GetNode(position.y, position.x);
		}

		public FungeNode GetOrCreateNode(int row, int column, int value=' ')
		{
			EnumerationArea = new FungeSpaceArea(row, column, FungeSpaceArea.FS_THEORETICAL.bottom, FungeSpaceArea.FS_THEORETICAL.right);
			foreach (var node in this)
			{
			...
			EnumerationArea = new FungeSpaceArea(row, column, FungeSpaceArea.FS_THEORETICAL.bottom, FungeSpaceArea.FS_THEORETICAL.right);
			return InsertCell(column, row, value);
		}

		public FungeNode GetOrCreateNode(Vector2 position, int value=' ')
		{
			return GetOrCreateNode(position.y, position.x, value);
		}
		...
		/// <returns></returns>
		public bool GetRealWorldBounds(ref Vector2[] bounds)
		{
		...
		public FungeNode InsertCell(FungeCell cell)
		{
			...
			if (spine_node == null)
			{
				...
				//We must start back at the Origin becase the row_node is not connected to anything
				spine_node = InsertRow(anchor_cell, Origin);
				Nodes.Add(spine_node);
		...
		public IEnumerator<FungeNode> GetEnumerator()
		{
			FungeSparseMatrixRowEnumerator _row_enumerator = new FungeSparseMatrixRowEnumerator(Origin, EnumerationArea.top, EnumerationArea.bottom);
		...
		public void PrintFungeSpace()
		{
			...
			foreach (var i in Origin.ParentMatrix)
			{
				if (i.Data.x == 0)
				{
					Console.WriteLine();
